...
//    private final List<Field> allFields;
    ...
    private boolean forceReferences = true;

    public DBAcess(String objectName, List<FieldDAO> fields) {
        this.objectName = objectName;
        fields.stream().filter(p->!p.getSaburiKey().equalsIgnoreCase(Enums.Saburikeys.UI_Only.name())).toList();
        this.fields = fields;
...
//        this.allFields.addAll(defaulFields);
    ...
    public String getObjectNameDA() {
     ...
     private String referencesDAGetter(FieldDAO fieldDAO) {
         String variableName = fieldDAO.getVariableName();
        return Utilities.makeMethod("public", fieldDAO.getReferencesDA(), "get" + fieldDAO.getFieldName() + "DA", "", 
                "return this." + variableName + "!=null? new " + fieldDAO.getReferencesDA() + "(this." + variableName + "):null;");
    }
    
     private String makeDaSetter(String objectName, FieldDAO fieldDAO) {
        String type = fieldDAO.getDataType();
        String variableName = fieldDAO.getVariableName();
        ...
        String setCall = fieldDAO.setCall(objectName);
...
//            return setCall(objectName)+makeSetter();
            return "public ".concat("void").concat(" ").
                    concat("set").concat(fieldName).concat("(" + declarationTrueFalse+ "){\n").concat(fieldDAO.setCall(objectName) + "this.").
                    concat(variableName).concat(" = ").concat(variableName).concat(";\n}") + ""
                    + " public void set" + fieldName + "DAs(List<" + referenceDA+ "> " + referenceVariableName + "DAs) {\n"
                    + "        this." + variableName + ".set" + fieldName + "(" + referenceDA + ".get" + references + "List(" + referenceVariableName + "DAs));\n"
                    + "\n"
                    + "    }";
            ...
            return Utilities.makeMethod("public", "void", "set" + fieldName, declarationTrueFalse,
                    setCall + "this." + variableName.concat(" = ").concat(variableName).concat(";"))
                    + Utilities.makeMethod("public", "void", "setImv" + fieldName, fieldDAO.getControlType() + " " + controlName,
                            "this." + controlName.concat(" = ").concat(controlName).concat(";"));
        } else if (type.equalsIgnoreCase("float") || type.equalsIgnoreCase("double")) {
            return Utilities.makeMethod("public", "void", "set" + fieldName, declarationTrueFalse,
                    fieldDAO.setCall(objectName) + "this." + variableName.concat(".set(" + variableName + ");\n"
                    + "this." + displayVariableName.concat(".set") + "(formatNumber(" + variableName + "));"));

        } else if (type.equalsIgnoreCase("int") || type.equalsIgnoreCase("Integer")) {
            return Utilities.makeMethod("public", "void", "set" + fieldName, declarationTrueFalse,
                    setCall+ "this." + variableName.concat(".set(" + variableName + ");\n"
      ...
      public String makeDaGetter(String objectName, FieldDAO fieldDAO) {
        ...
        String references = fieldDAO.getReferences();
        String referenceDA = references.concat("DA");
        String referencesID = references.concat("ID");
        String referencesVariableID =  Utilities.getVariableName(referencesID);
       String controlName = fieldDAO.getControlName();
        ...
        if (isCollection) {
            return fieldDAO.makeGetter(forceReferences) + ""
                    + "public List<" + referenceDA + "> get" + fieldName + "DAs(){\n"
                    + "       return " + referenceDA+ ".get" + referenceDA + "s(" + objectVariableName + "." + getCall + ");\n"
                    + "    }";
        } else if (isReferences&& !isCollection) {
            if (fieldDAO.getEnumerated()) {
                return Utilities.makeMethod("public", "Object", "get" + fieldName, "", "return " + variableName.concat(".get();"));
            } else {
                String getRef = Utilities.makeMethod("public", references, "get" + fieldName, "", "return " + variableName + ";");

                String getRefID = Utilities.makeMethod("public", "Object", "get" + referencesID, "", "return " + referencesVariableID.concat(".get();"));
                String getRefDis = Utilities.makeMethod("public", "String", "get" + display, "", "return " + displayVariableName.concat(".get();"));

                return getRef + getRefID + getRefDis + referencesDAGetter(fieldDAO);
            }
        }
        if (type.equalsIgnoreCase("Object")) {
            ...
            return Utilities.makeMethod("public", "Object", "get" + fieldName, "", "return " + variableName.concat(".get();"))
                    + Utilities.makeMethod("public", displayDataType, "get" + display, "", "return " + displayVariableName.concat(".get();"));
        } else if (type.equalsIgnoreCase("float") || type.equalsIgnoreCase("double") || type.equalsIgnoreCase("int") || type.equalsIgnoreCase("Integer")) {
            return Utilities.makeMethod("public", usableDataTypeFalse, "get" + fieldName, "", "return " + variableName.concat(".get();"))
                    + Utilities.makeMethod("public", displayDataType, "get" + display, "", "return " + displayVariableName.concat(".get();"));
        } else if (type.equalsIgnoreCase("bool") || type.equalsIgnoreCase("boolean")) {
            return Utilities.makeMethod("public", usableDataTypeFalse, "is" + fieldName, "", "return " + variableName.concat(".get();"));
        } else if (type.equalsIgnoreCase("Image")) {
            return fieldDAO.makeGetter(forceReferences) + "\n"
                    + Utilities.makeMethod("public ", "ImageView", "getImv" + fieldName, "", "return " + controlName + ";");
        } else {
            return Utilities.makeMethod("public", usableDataTypeFalse, "get" + fieldName, "", "return " + variableName.concat(".get();"));
 ...
 public String makeDAProperties(String objectName, FieldDAO fieldDAO) {
        return makeDaGetter(objectName, fieldDAO) + this.makeDaSetter(objectName, fieldDAO);
    }

    public String makeImports(Project currentProject) throws Exception {
        ...
        if (!commonProject.getProjectName().equalsIgnoreCase(currentProject.getProjectName())) {
            imp += "import " + oProjectDAO.get(currentProject.getCommonProjectName()).getDBAccessPackage() + ".DBAccess;\n";
        }
...
//                }
    ...
    public String makeDAProperties() {
        String properties = "";
        properties = fields.stream().map((field) -> this.makeDAProperties(objectName, field)).reduce(properties, String::concat);
...
//    }
    ...
    public String makeClass(Project project) throws Exception {
        ...
        JavaClass javaClass = new JavaClass(project.getDBAccessPackage(), objectNameDA, this.makeImports(project),
